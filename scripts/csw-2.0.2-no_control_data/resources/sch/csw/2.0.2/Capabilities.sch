...
<sch:schema xmlns:sch="http://www.ascc.net/xml/schematron" 
  ...
  <sch:pattern id="RequiredParametersPattern" name="RequiredParametersPattern">
    ...
    <!-- I comment this rule out, since those parameters are not mandatory.
      According to the specification (§10.5.4, p.133): "each of the GetCapabilities operation request parameters might
      have its domain recorded. For example, the domain of the Sections parameter could record all the sections implemented by that specific server."  -->
<!--   <sch:rule id="RequiredGetCapabilitiesParameters" context="//ows:OperationsMetadata/ows:Operation[@name='GetCapabilities']">
   ...
   <sch:rule id="RequiredDescribeRecordParameters" context="//ows:OperationsMetadata/ows:Operation[@name='DescribeRecord']">   
      <sch:assert id="DescribeRecordTypeNames" 
	...
	Missing mandatory outputFormat parameter for DescribeRecord, "application/xml".
      ...
      <sch:assert id="DescribeRecordSchemaLanguage" 
        test="ows:Parameter[translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')='schemalanguage']/ows:Value = 'XMLSCHEMA'">
	Missing mandatory schemaLanguage parameter for DescribeRecord, "XMLSCHEMA".
      </sch:assert>       
   </sch:rule>
